...
### GEDCOM 7.0 & Phát hiện trùng lặp (v2.6)

- **GEDCOM 7.0 Export** - Nâng cấp từ 5.5.1, hỗ trợ Vietnamese extensions (Quá Kế)
- **Phát hiện trùng lặp** - Thuật toán Levenshtein + Vietnamese NFD normalization + blocking strategy
- **Admin review queue** - `/admin/duplicates`: xem, merge, bỏ qua thành viên trùng
- **Chế độ người cao tuổi** - Font size toggle (CSS custom property) + simplified list view

### Tìm kiếm & Thống kê (v2.7)
...
- **Thông báo realtime** - Bell icon + dropdown + trang thông báo đầy đủ
- **6 loại thông báo** - Hệ thống, sự kiện, bài viết mới, bình luận, thích, nhắc nhở
- **Đọc/xoá** - Đánh dấu đã đọc, xoá từng thông báo
- **DB triggers** - Tự động tạo thông báo khi có sự kiện (không insert trực tiếp)
- **RLS bảo vệ** - Mỗi user chỉ thấy/xoá thông báo của mình

### Export/Import (v2.9)

- **GEDCOM Import** - Parse .ged 5.5.1/7.0 → tạo people + families tự động
- **CSV Export** - Xuất danh sách thành viên ra Excel/CSV
- **Validation** - Kiểm tra dung lượng file, format, duplicate trước import
- **Desktop support** - Hoạt động offline với SQLite
...
- **Lịch tế lễ** - Quản lý từ admin settings, hiển thị trên trang công khai
...
│   │   │       ├── notifications/ # Thông báo (v2.8)
│   │   │       ├── people/         # Quản lý thành viên
│   │   │       ├── relationship/  # Tìm quan hệ (v2.7)
...
| Notifications (v2.8) | `notifications` | Thông báo realtime (trigger-only insert) |
| Registration (v3.0) | `member_registrations` | Đơn ghi danh thành viên |
...
v2.6.0 GEDCOM    [##########] Done - GEDCOM 7.0 Export + Phát hiện trùng lặp + Chế độ người cao tuổi
v2.7.0 Search    [##########] Done - Tìm quan hệ + Thống kê + Xuất PDF
v2.8.0 Notify    [##########] Done - Thông báo realtime (bell + triggers)
v2.9.0 Export    [##########] Done - GEDCOM Import + CSV Export
v3.0.0 Community [##########] Done - Nhà thờ họ + Đăng ký thành viên + SEO
```
